...
namespace VOOQQ_APP.Controllers
{
    public class AuditoriumsController : Controller
    {
        ...
        // GET: Auditoriums
        public ActionResult Index(int? Vid, string Location, string AuditoriumCategoryId)
        {


            ViewBag.AuditoriumCategory = db.AuditoriumCategory.ToList();
            Vid = Vid == null ? 1 : Vid;
            AuditoriumCategoryId = AuditoriumCategoryId == null || AuditoriumCategoryId == "" ? "0" : AuditoriumCategoryId;
           
            SqlParameter[] Parameters = {
                new SqlParameter("@Vid", Vid),
                new SqlParameter("@Location", string.IsNullOrWhiteSpace(Location)? DBNull.Value: (object)Location),
                new SqlParameter("@AuditoriumCategoryId",  AuditoriumCategoryId)
           
            };
            ViewBag.data = db.Database.SqlQuery<AuditoriumModel>("[dbo].[spGetListingData_Auditorium] @Vid, @Location, @AuditoriumCategoryId", Parameters).ToList();
        ...
        public ActionResult Create([Bind(Include = "AuditoriumId,VId,UserId,PropertyName,AuditoriumCategoryId,Location,Price,SeatingCapacity,DiningCapacity,ParkingCapacity,AuditoriumTypeId,MobileNumber,LandPhone,Address,Email,Website,About,Image1,Image2,Image3,Date,Views,Status")] Auditoriums auditoriums, HttpPostedFileBase file1, HttpPostedFileBase file2, HttpPostedFileBase file3)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                auditoriums.UserId = User.Identity.GetUserId();
                auditoriums.Date = DateTime.Now;
                auditoriums.StratDate = DateTime.Now;
