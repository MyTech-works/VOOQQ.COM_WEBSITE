...
namespace VOOQQ_APP.Controllers
{
    public class JobsController : Controller
    {
        ...
        // GET: Jobs
        public ActionResult Index(int? Vid, string Location, string JobCategoryId, string PostedById)
        {
            ViewBag.JobCategory = db.JobCategory.ToList();
            ViewBag.JobPostedBy = db.JobPostedBy.ToList();
            Vid = Vid == null ? 1 : Vid;
            JobCategoryId = JobCategoryId == null || JobCategoryId == "" ? "0" : JobCategoryId;
            PostedById = PostedById == null || PostedById == "" ? "0" : PostedById;
            SqlParameter[] Parameters = {
                new SqlParameter("@Vid", Vid),
                new SqlParameter("@Location", string.IsNullOrWhiteSpace(Location)? DBNull.Value: (object)Location),
                new SqlParameter("@JobCategoryId",  JobCategoryId),
                new SqlParameter("@PostedById",  PostedById),
            };
            ViewBag.data = db.Database.SqlQuery<JobModels>("[dbo].[spGetListingData_Job] @Vid, @Location, @JobCategoryId, @PostedById", Parameters).ToList();
            return View();
        }
        ...
        public ActionResult Details(string id)
        {
            id = id == null ? "" : id;
            SqlParameter[] Parameters = {
                    new SqlParameter("@JobId", id.ToString())};
            ViewBag.data = db.Database.SqlQuery<JobModels>(" [dbo].[spGetListingData_Job_Details] @JobId", Parameters).ToList();

            long ids = Convert.ToInt64(id);
            Job forsale = db.Jobs.Find(ids);
        ...
        public ActionResult Create()
        {
            ViewBag.JobCategory = db.JobCategory.ToList();
            ViewBag.JobPostedBy = db.JobPostedBy.ToList();
            ViewBag.JobStatus = db.JobStatus.ToList();
            ViewBag.JobLookingfor = db.JobLookingfor.ToList();
            return View();
        }
        ...
        public ActionResult Create([Bind(Include = "views,status,JobId,VId,UserId,Title,JobCategoryId,Salary,PostedById,StatusId,LookingforId,MobileNumber,LandPhone,Email,Description,Location,Image1,Image2,Image3,date")] Job job, HttpPostedFileBase file1, HttpPostedFileBase file2, HttpPostedFileBase file3)
        {
            ViewBag.JobCategory = db.JobCategory.ToList();
            ViewBag.JobPostedBy = db.JobPostedBy.ToList();
            ViewBag.JobStatus = db.JobStatus.ToList();
            ...
            if (ModelState.IsValid)
            {
                    ...
                    using (Bitmap bmp = new Bitmap(file1.InputStream, false))
                    {
                        using (Graphics grp = Graphics.FromImage(bmp))
                        {
                            ...
                            using (MemoryStream memoryStream = new MemoryStream())
                            {
                                ...
                                job.Image1 = "/Images/" + file1.FileName;
                            }
                        }
                    }
                }
                ...
                {
                    job.Image1 = "/Images/nullimage.jpg";
                }
                if (file2 != null)
                {
                    string watermarkText = "© Vooqq.Com";
                    ...
                    using (Bitmap bmp = new Bitmap(file2.InputStream, false))
                    {
                        using (Graphics grp = Graphics.FromImage(bmp))
                        {
                            ...
                            using (MemoryStream memoryStream = new MemoryStream())
                            {
                                ...
                                job.Image2 = "/Images/" + file2.FileName;
                            }
                        }
                    }
                }
                ...
                {
                    job.Image2 = "/Images/nullimage.jpg";
                }
                if (file3 != null)
                {
                    string watermarkText = "© Vooqq.Com";
                    ...
                    using (Bitmap bmp = new Bitmap(file3.InputStream, false))
                    {
                        using (Graphics grp = Graphics.FromImage(bmp))
                        {
                            ...
                            using (MemoryStream memoryStream = new MemoryStream())
                            {
                                ...
                                job.Image3 = "/Images/" + file3.FileName;
                            }
                        }
                    }
                }
                ...
                {
                    job.Image3 = "/Images/nullimage.jpg";
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "UserId,EndDate,status,StratDate,views,status,JobId,VId,UserId,Title,JobCategoryId,Salary,PostedById,StatusId,LookingforId,MobileNumber,LandPhone,Email,Description,Location,Image1,Image2,Image3,date")] Job job, HttpPostedFileBase file1, HttpPostedFileBase file2, HttpPostedFileBase file3)
        {
            ViewBag.JobCategory = db.JobCategory.ToList();
            ViewBag.JobPostedBy = db.JobPostedBy.ToList();
            ViewBag.JobStatus = db.JobStatus.ToList();
